...
def tocar_bip(config: dict[str, Any]) -> None:
    """Toca o bip de wake word. Não-bloqueante (sd.play retorna imediatamente)."""
    ecfg = config.get("voice", {}).get("earcons", {})
    freq = ecfg.get("wake_freq_hz", 880)
    dur = ecfg.get("wake_duration_ms", 120)
    amp = ecfg.get("wake_amplitude", 0.2)

    try:
        tom = _gerar_tom(freq, dur, amp)
        sd.play(tom, _EARCON_SAMPLE_RATE)
    except Exception as exc:  # noqa: BLE001
        LOGGER.warning("Falha ao tocar bip: %s", exc)
